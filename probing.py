...
class Cycle:
    ...
    def __init__(self):
        ...
        if self.body[0] not in [1001, 1002, 1003, 1004, 0, 1, 2, 3, 4]:    # if set zero point
            while 1:
                x = self.__enter_int('Enter offset [G54-57, G508-G599]: G')
        ...
        self.body.append(self.__enter_int('Enter set of calibration data [1-10]:'))
        ...
        # safe distance
        if self.body[0] in [0,100]: self.body.append(self.__enter_float('Enter travel distance [mm]:'))
        else: self.body.append(self.__enter_float('Enter safety distance DFA [mm]:'))

        # contingence interval
        self.body.append(self.__enter_int('Enter contingence interval [s]:'))

        # probing angle
        if self.name == 977:
            self.body.append(self.__enter_float('Enter probing angle along with axe [deg]:'))
